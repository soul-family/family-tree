...
def load_paths_to_replace(script_dir, developer_id=None):
    ...
    Canonical location: .dev-scripts/paths-to-replace.jsonc
    Falls back to developer config dir and script dir for backward compatibility.
    """
    patterns = []
    
    # Primary: .dev-scripts/paths-to-replace.jsonc
    dev_scripts_root = os.path.dirname(os.path.dirname(script_dir))
    primary = os.path.join(dev_scripts_root, 'paths-to-replace.jsonc')
